...
const thumbsDirectory: string = join(currentDirectory, '..', '..', '..', 'imagesResized')

const imagesProcessingRouter: Router = express.Router()

imagesProcessingRouter.get('/', async (request: Request, response: Response): Promise<void> => {
    const images: string[] = [
    'encenadaport',
    'fjord',
    'icelandwaterfall',
    ...
    try {
      // Api started
        ...
        if (await imagesProcessing(originalImageName, height, width)) {
            ...
            const imageContent: Buffer = fs.readFileSync(imageResized, { flag: 'r' })
